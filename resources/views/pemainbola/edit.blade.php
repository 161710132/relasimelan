	...
	<div class="container">
		<div class="col-md-12">
			<div class="panel panel-primary">
			  <div class="panel-heading">Edit Data Pemain Bola 
			  	<div class="panel-title pull-right"><a href="{{ url()->previous() }}">Kembali</a>
			  	</div>
			  </div>
			  <div class="panel-body">
			  	<form action="{{ route('pemainbola.update',$c->id) }}" method="post" >
			  		<input name="_method" type="hidden" value="PATCH">
        			{{ csrf_field() }}
			  		<div class="form-group {{ $errors->has('asal_kota') ? ' has-error' : '' }}">
			  			<label class="control-label">Asal Kota</label>	
			  			<input type="text" name="asal_kota" value="{{ $c->asal_kota }}" class="form-control"  required>
			  		...
			  		<div class="form-group {{ $errors->has('clubbola_id') ? ' has-error' : '' }}">
			  			...
			  			<select name="clubbola_id" class="form-control">
			  				@foreach($mhs as $data)
			  				<option value="{{ $data->id }}" {{ $selectedclubbola == $data->id ? 'selected="selected"' : '' }} >{{ $data->nama_club }}</option>
			  				@endforeach
			  			</select>
